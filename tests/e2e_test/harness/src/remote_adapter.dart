...
if ! command -v sqlite3 >/dev/null 2>&1; then
  echo "E2E_STAGE sqlite3 missing"
  exit 0
fi
...
if [ -n "$USER_DID" ] && [ -n "$APP_INSTANCE" ] && [ "$(q "SELECT COUNT(*) FROM user_delegated_identity WHERE user_did='$USER_DID' AND app_instance_id='$APP_INSTANCE' AND status='paired_key_received';")" != "0" ]; then
  echo "E2E_STAGE delegated_key_imported pass"
else
  echo "E2E_STAGE delegated_key_imported missing"
fi
if [ -n "$USER_DID" ] && [ -n "$APP_INSTANCE" ] && [ "$(q "SELECT COUNT(*) FROM app_message_agent_binding WHERE user_did='$USER_DID' AND app_instance_id='$APP_INSTANCE' AND role='app_message_handler' AND revoked_at_ms IS NULL AND status IN ('message_agent_ready','message_agent_active','message_agent_ensuring');")" != "0" ]; then
  echo "E2E_STAGE hermes_agent_ready pass"
else
  echo "E2E_STAGE hermes_agent_ready missing"
fi
if [ "$(q "SELECT COUNT(*) FROM processed_message WHERE message_id='$MSG_ID';")" != "0" ] || [ "$(q "SELECT COUNT(*) FROM message_event WHERE message_id='$MSG_ID';")" != "0" ]; then
  echo "E2E_STAGE cli_message_received pass"
else
  echo "E2E_STAGE cli_message_received missing"
fi
if [ "$(q "SELECT COUNT(*) FROM runtime_task t JOIN runtime_run r ON r.task_id=t.task_id WHERE t.text LIKE '%$MSG_ID%' AND r.status='finished';")" != "0" ]; then
  echo "E2E_STAGE hermes_runtime_finished pass"
else
  echo "E2E_STAGE hermes_runtime_finished missing"
fi
if [ "$(q "SELECT COUNT(*) FROM message_sync_outbox WHERE status='sent' AND payload_json LIKE '%runtime_final%' AND payload_json LIKE '%$MSG_ID%';")" != "0" ]; then
