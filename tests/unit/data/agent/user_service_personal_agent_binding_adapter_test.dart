...
void main() {
  ...
  test(
    'falls back to the legacy endpoint only when canonical is missing',
    () async {
      final httpClient = _QueueHttpClient(<http.Response>[
      ...
      expect(binding.personalAgentDid, 'did:agent:personal');
      expect(httpClient.requests.map((request) => request.url.path), <String>[
        UserServicePersonalAgentBindingAdapter.endpoint,
      ...
      expect(
        (legacyParams['runtime_profile'] as Map)['profile'],
        'message_agent',
      );
    },
  );
  ...
  test(
    'does not replay canonical validation errors through legacy API',
    () async {
      final httpClient = _QueueHttpClient(<http.Response>[
        http.Response(
  ...
  return jsonEncode(<String, Object?>{
    ...
    'result': <String, Object?>{
      'binding': <String, Object?>{
        ...
        'delegated_key_verification_method': 'did:human:alice#daemon-key-1',
        'status': 'active',
      },
    },
    'id': 'req-1',
