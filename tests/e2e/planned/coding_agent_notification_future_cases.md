...
- Scope: real Coding Agent or Skill Agent → daemon/CLI → User Service and
  Message Service → AWiki Me foreground Toast, background macOS notification,
  visible recent conversation, and exact-once terminal semantics.
- Blocker: the current AWiki Me checkout has no sibling `awiki-system-test`
  workspace or runner fixture that can provision isolated App, CLI, daemon, and
  coding-runtime identities on one approved tenant and correlate the ordinary
  final message with `awiki.agent.status.v1`.
