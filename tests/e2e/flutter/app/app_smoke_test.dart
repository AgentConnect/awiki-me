...
void main() {
  ...
  testWidgets('AwikiMeApp shows Coding Agent terminal feedback', (
    tester,
  ) async {
    const session = SessionIdentity(
      did: 'did:test:me',
    ...
    expect(find.text('个人助理已完成处理: 通知 Smoke 已完成'), findsOneWidget);
    expect(harness.notificationFacade.systemNotificationCount, 0);
    await E2eCaseAttestationWriter.markPassed(
      'AGENT-NOTIFY-SMOKE-E2E-001',
      phases: const <String>[
        'terminal_status_reached_app_shell',
        'foreground_terminal_toast_visible',
        'foreground_system_notification_not_used',
      ],
    );
