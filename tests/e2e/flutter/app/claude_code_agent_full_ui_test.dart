  ...
  testWidgets(
    ...
    (tester) async {
      ...
      try {
        ...
        final runtimeHandle = _claudeCodeRuntimeHandle(config.runId);
        ...
        await _pumpUntil(
          ...
          description: 'Claude Code runtime create action to finish',
        );
        ...
        if (stateAfterCreate.error != null) {
          fail(
            'Claude Code runtime create failed: ${stateAfterCreate.error}. '
        ...
        await _waitForVisibleClaudeCodeReply(
          tester: tester,
          expectedReply: config.expectedReply,
        );
        await E2eCaseAttestationWriter.markPassed(
...
Future<AppSession> _prepareRealAppIdentity(
  ...
  final register = await _tryAppIdentityAction(
    () => onboarding.registerHandleWithPhone(
      ...
      nickName: 'Claude Code Agent E2E ${config.runId}',
    ),
  );
...
Future<_DaemonInstallResult> _installRealDaemon({
  required _ClaudeCodeAgentRealBackendConfig config,
...
Map<String, String> _daemonEnvironment(
  _ClaudeCodeAgentRealBackendConfig config,
) {
...
Map<String, String> _loadDaemonEnvFile(
  _ClaudeCodeAgentRealBackendConfig config,
) {
...
Future<void> _waitForDaemonGenericCliCapability({
  ...
  while (DateTime.now().isBefore(deadline)) {
    ...
    if (daemon != null && _daemonSupportsClaudeCode(daemon)) {
      return;
    }
    await Future<void>.delayed(const Duration(seconds: 1));
  }
...
Future<AgentSummary> _waitForRuntimeAgentByHandle({
  ...
  while (DateTime.now().isBefore(deadline)) {
    ...
    if (matchingRuntimes.length > 1) {
      fail(
        'Expected one canonical Claude Code runtime for handle=$handle, found '
  ...
  fail(
    'Timed out waiting for Claude Code runtime handle=$handle. '
    'Last agents: ${lastState ?? '<none>'}',
  );
}
...
Future<void> _expectSingleRuntimeAgentRow({
  ...
  await _pumpUntil(
    ...
    description: 'canonical Claude Code runtime row to become visible',
  );
...
Future<void> _sendPromptThroughUi(WidgetTester tester, String prompt) async {
  ...
  await _pumpUntil(
    ...
    description: 'Claude Code chat input to become visible',
  );
...
Future<void> _waitForDaemonClaudeCodeFinalSent({
  ...
  await _poll(
    description: 'daemon sent Claude Code runtime final reply "$expectedReply"',
    action: () async {
      ...
      try {
        final rows = await db.rawQuery(
          ...
          SELECT t.task_id, t.status AS task_status,
                 ...
                 s.last_error_code AS route_last_error_code,
                 s.last_error_summary AS route_last_error_summary
          FROM runtime_task t
          ...
          LEFT JOIN cli_route_sessions s ON s.last_run_id = r.run_id
...
Future<ChatMessage> _waitForAppIncomingClaudeCodeReply({
  ...
  Object? lastState;
  await _poll(
    description: 'App local history contains incoming Claude Code reply',
    action: () async {
...
Future<void> _waitForVisibleClaudeCodeReply({
  required WidgetTester tester,
  required String expectedReply,
}) async {
...
bool _daemonSupportsClaudeCode(AgentSummary daemon) {
  ...
  return schemaVersion == 1 && drivers.contains('claude-code');
}
...
String _claudeCodeRuntimeHandle(String runId) {
  final suffix = runId
      .toLowerCase()
  ...
  final handle = 'claude-e2e-${suffix.isEmpty ? 'run' : suffix}-$nonce';
  if (handle.length <= 63) {
    return handle;
  }
...
class _ClaudeCodeAgentRealBackendConfig {
  const _ClaudeCodeAgentRealBackendConfig({
    required this.runId,
  ...
  static _ClaudeCodeAgentRealBackendConfig? tryLoad() {
    ...
    if (raw is! Map) {
      throw StateError('$_claudeCodeAgentRunConfigPath must be a JSON object.');
    }
    final map = _stringKeyMap(raw, path: _claudeCodeAgentRunConfigPath);
    final claudeCodeAgent = _optionalMapAt(map, 'claudeCodeAgent');
    final enabled = _boolConfig(claudeCodeAgent, 'enabled');
    final realBackend = _boolConfig(claudeCodeAgent, 'realBackend');
    ...
    final expectedReply =
        _optionalConfig(claudeCodeAgent, 'expectedReply') ??
        _defaultClaudeCodeExpectedReply(runId);
    return _ClaudeCodeAgentRealBackendConfig(
      runId: runId,
      ...
      daemonHandle:
          ...
          'claude-code-agent-daemon-${DateTime.now().millisecondsSinceEpoch}',
      daemonEnvFile: _optionalConfig(daemon, 'envFile'),
      enabled: enabled,
      realBackend: realBackend,
      prompt:
          _optionalConfig(claudeCodeAgent, 'prompt') ??
          _defaultClaudeCodePrompt(runId),
...
class _ProcessResult {
  ...
  String sanitizedSummary(_ClaudeCodeAgentRealBackendConfig config) {
...
String _requiredConfig(Map<String, Object?> map, String key, String name) {
  ...
  if (value == null) {
    throw StateError('$name is required in $_claudeCodeAgentRunConfigPath.');
  }
  return value;
}
