...
部分 metric 名称保留 `thread` 字样是历史兼容，不表示 `ThreadRef` 是当前消息显示 correctness key。当前判断可见性和首屏性能时，仍以 `ConversationSummary.conversationId` / `AppConversationReadRef` 打开的 canonical conversation timeline 为准。

first-paint gate 必须先等待 fast local conversation preview，然后使用
`ConversationSummary.conversationId` 打开会话，通过 `AppConversationReadRef`
