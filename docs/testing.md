...
`DEVICE-JOIN-E2E-002`、`ROOT-TRANSFER-E2E-001` 和 `DEVICE-REVOKE-E2E-001`
已由独立的 `multi-device-remote-join` suite 承载；它们不会混入本地 capability gate。
该 suite 使用真实 AWiki Me 管理设备和独立 CLI 请求设备、动态一次性 OTP、双端 SAS、
设备级 Direct 根控制、imported ACK、Registry readiness/revoke、真实 macOS
user-presence 与场景级 attestation。当
...
staged SMS error 不构成该产品 gate。`HANDLE-RECOVERY-E2E-003`、
`DEVICE-JOIN-E2E-001/003`、`ROOT-TRANSFER-E2E-002` 和 `MLS-MULTI-DEVICE-E2E-*` 仍为
planned；不得把本地 capability gate、Widget fake 或手工演示记录为这些远端用例的 E2E pass。
...
Run the activation-gated remote App-admin + CLI management lifecycle only
