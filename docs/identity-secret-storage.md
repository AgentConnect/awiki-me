...
生产和普通 custom state root 使用 `SecureAppKeyValueStore`，底层为
`flutter_secure_storage`。App 只保存打开 vault 所需的最小 host secret/context：
...
`awiki_me_im_core_vault.json` 私有 file test provider。这个 JSON 可能包含 base64
root key，必须留在本地并保持 untracked。
...
2. 从 `StoredAwikiImCoreVaultSecretProvider` 获取或创建 root key 和 device id。
3. 用 `AwikiImCoreOpenOptions.vaultRequired(...)` 打开 `AwikiImCore`。

如果 root key 损坏、长度不对，或 strict file provider 已存在但缺 key，App 必须 fail
closed，不重新生成一个新 root key 覆盖已有状态。
...
`awiki_me_im_core_vault.json` 保存 App-local root key 和 device id。该文件在 E2E
状态目录下，必须保持本地私有。
