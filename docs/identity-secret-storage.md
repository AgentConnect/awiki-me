...
> **首发契约说明**：typed Storage Scope envelope 和平台 provider 已实现，详见
> [Scope Secret Platform Provider](scope-secret-platform.md)；App runtime 仍待下一步从
> namespace-scoped 预发布 provider 切换。长期契约见
> [Storage Scope / Keychain / Vault Contract](storage-scope-vault-contract.md)。
...
以上是尚待删除的 runtime path，不是首发存储格式。新的 scope provision control plane 已使用
typed `ScopeSecretRepository`，其 envelope、exclusive create、CAS 和平台隔离不读取上述
namespace/split/bundle item；runtime 接入完成前默认 provider 会 fail closed，而不是回退旧格式。
...
## 2. 预发布 Runtime Root Key Provider（待 cutover 删除）
...
该预发布 provider 不迁移、不读取、也不删除更早的拆分 key：
...
旧 runtime 只有显式 E2E mode，也就是设置 `AWIKI_E2E_APP_STATE_ROOT` 时，才使用
...
首发 typed provider 的 production/development locator、iOS/Android选项、macOS bridge 和
E2E 0600 file provider见 [Scope Secret Platform Provider](scope-secret-platform.md)。它没有
`getOrCreate`；只有显式 provision 可以调用 `createExclusive`。
