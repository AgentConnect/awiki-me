...
- The Agent/Daemon realm allowlist contains the current official realms `awiki.ai` and `awiki.me` plus the compatibility realms `agent-connect.cn`, `awiki.info`, and `anpclaw.com`; compatibility realms stay enabled until a server-capability migration explicitly replaces them. Skill onboarding also requires an explicit same-origin server-info capability.
