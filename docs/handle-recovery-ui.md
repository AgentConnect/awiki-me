...
状态：App-owned 状态机、生产 Core adapter 与 requester 激活链已接入；默认关闭，旧管理设备通知消费仍阻断
...
旧管理设备从 Core 的本地恢复通知投影读取 `oldAdmin + canCancelFromThisDevice=true` 状态，
在“设备”页获得显式取消入口。新设备不能把本地放弃误当成服务端取消。
...
Core `cancel` 只返回权威的 Session ID 和 phase。Application service 仅用这两个字段和调用前已
验证的 current progress 生成 cancelled UI projection，不从 adapter 猜测 Handle、旧 DID 或时间。
当前 Core 尚未消费 `awiki.identity.recovery-started.v1` 来建立旧管理设备的 durable local
projection，因此旧管理设备自动发现并取消恢复仍是启用阻断，不能宣称已经完成。
...
3. finalize 在新 Storage/Owner Scope 内生成全新 DID 和设备密钥，绝不复制旧 Ratchet、MLS
   或 root secret；
4. 旧管理设备通知、签名取消和 cancel/finalize CAS 竞争；
...
fallback。特别是旧管理设备 recovery-started durable consumer 完成前，整体 Recovery capability
仍不能发布启用。
