...
状态：App-owned 状态机和 UI 已实现；生产 Core adapter 未接入，默认关闭
...
入口由编译期开关 `AWIKI_HANDLE_RECOVERY_ENABLED` 控制，默认 `false`。当前仓库没有生产
Recovery adapter；即使服务端声明支持恢复，开关关闭时也不会退回旧版立即恢复，且不会
改变身份状态。

启用前必须由 Core/SDK 提供并验证：
