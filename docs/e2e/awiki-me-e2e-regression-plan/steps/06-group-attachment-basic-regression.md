...
- [ ] 群组创建和群内双向文本消息有 runId evidence。
- [ ] 附件双向发送/接收至少覆盖一个小型 fixture。
- [ ] Agent 作为 IM App 处理者的互通场景在主矩阵中标记为 `Skipped`，不要求相关验证证据。
- [ ] 本轮端到端加密在主矩阵中标记为 `Skipped`，不要求相关验证证据。
- [ ] remote/log/report 经过 redaction scan。
- [ ] 不把 remote unavailable 伪装为 pass。
- [ ] Review 发现已经修复或明确记录。
