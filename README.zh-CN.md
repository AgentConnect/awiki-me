...
- macOS Intel DMG；
- Windows x64 EXE，支持原生 x64 Windows 与 Windows 11 ARM64 的 x64 应用模拟。

正式对外发布前，请在这里补充经过验证的官方下载入口。Windows 安装器当前未签名，可能显示 SmartScreen 警告；不要把内部包地址或临时 CI artifact 作为公开下载链接。
...
| Windows | 打包与验证目标 | 同一个 x64 EXE 在 x64 Windows 上原生运行，并可通过 Windows 11 ARM64 的 x64 应用模拟运行；当前不生成原生 ARM64 包 |
| iOS | 开发目标 | 工程与 SDK 原生入口存在，但当前默认打包脚本不产出 iOS 发布包 |
| Web | 当前不可用 | `awiki_im_core` 的 Web 入口运行时抛出 `UnsupportedError` |
| Linux App | 当前未声明为产品目标 | Rust/Dart 核心能力与完整 App 平台支持不能混为一谈 |
