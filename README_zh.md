...
- **联系人与资料**：好友 / 关系状态、peer profile、按租户与 owner 隔离的本地 Profile 展示投影、会话区域只展示 Display Name（handle 仅作为身份元数据）、复制与身份卡、关注/粉丝列表独立失败与重试（条目优先显示昵称、无昵称时回退到 handle），以及关注 / 取消关注后的乐观 UI 状态。会话和关系列表只读取本地投影；打开用户资料时才显式远端刷新并统一发布更新。
