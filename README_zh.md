...
| `ANPMessageService` 消息端点 | App 从 `AWIKI_BASE_URL` 推导默认 ANP 消息端点 `/anp-im/rpc` 与服务 DID `did:wba:<domain>`，也支持显式覆盖。 | `lib/src/application/config/awiki_environment_config.dart` |
...
| ANP 附件 / Object Transfer（P7 方向） | App 发起附件发送、下载、保存与本机打开；展示事实来自 im-core 持久化的 redacted attachment manifest，而不是仅靠 UI 内存。 | `lib/src/application/attachment_*`、`lib/src/presentation/chat/` |
...
- **群协作**：群创建、成员摘要、群消息、群系统事件展示、群 Mention 与 Agent 群协作入口。
- **联系人与资料**：好友 / 关系状态、peer profile、handle / DID 展示、复制与身份卡。
- **附件**：附件选择、上传/发送、下载、保存、本机应用打开与 E2E 互通验证。
...
App 读取一个 backend root，并从它推导 User Service、Message Service、Mail Service、DID domain、ANP endpoint、Daemon 下载地址和更新地址：
...
同一个 App 包内不要混用 `awiki.info`、`awiki.ai` 或其他域名的服务地址，避免 DID domain、ANP service DID、状态 namespace、daemon token 和下载通道不一致。
...
AWIKI_DOMAIN="awiki.info"  # 内部镜像 / 联调包常用
```

脚本会从 `AWIKI_DOMAIN` 推导 `AWIKI_BASE_URL`、DID 域名、ANP 服务、Daemon 下载地址、更新清单和下载页。高级覆盖项默认留空即可。
