...
- 设计基准 1：`/var/folders/2k/sbpv92td6qldrfzhbfs161_r0000gn/T/codex-clipboard-3d9ccd97-0a43-4bd0-803a-b078d230df35.png`，原始 `2560 × 1664`，用于确认智能体标题栏原有三个操作入口。
- 设计基准 2：`/var/folders/2k/sbpv92td6qldrfzhbfs161_r0000gn/T/codex-clipboard-ca35568a-4de6-4b40-8440-9e7a78bdcda4.png`，原始 `2560 × 1664`，用于确认单一圆形加号与锚定下拉列表的交互模式。
- 实现关闭态：`docs/ui-optimization-plan/screenshots/08-expanded-agents.png`，Flutter macOS expanded `1440 × 900 @1x`。
- 实现打开态：`docs/ui-optimization-plan/screenshots/08b-expanded-agent-actions.png`，Flutter macOS expanded `1440 × 900 @1x`；状态为智能体列表有数据、标题栏菜单打开。
- 密度归一：两张设计基准来自 Retina 截图，可视 CSS 尺寸约为 `1280 × 832`；实现截图为 `1440 × 900 @1x`。本轮只比较标题栏按钮与锚定菜单区域，未把当前产品内容数据和旧截图中的空状态当作视觉偏差。

### 全图与局部比较

- 全图：当前 AWiki Me 已演进为更紧凑的桌面信息架构，列表数据、详情卡片和侧栏宽度不是本次修改范围；标题栏保留独立刷新按钮，并把生成指令与安装代理收敛到圆形加号，未改变列表或详情布局。
- 局部：刷新与加号并列在智能体标题栏右侧；菜单从加号下方右对齐展开，两行使用同一图标、文字、行高、圆角、描边和阴影体系，操作项没有裁切、重叠或溢出。
- 重点区域只包含标题栏与两行菜单，`08b-expanded-agent-actions.png` 已可清晰检查全部字号、文案、图标和间距，无需额外放大裁切。

### 比较历史与修正

- 首轮 P2：默认 `200dp` 菜单宽度导致“生成 Skill Agent 安装指令”被省略号截断。已为可复用锚定菜单增加可选桌面宽度，并将智能体菜单设为 `240dp`。
- 第二轮 P2：初版把刷新也收进加号菜单，不符合补充确认后的信息架构。已恢复刷新为标题栏直接操作，加号菜单只保留生成指令与安装代理。
- 第三轮：完整文案可见，菜单仍保持在列表栏内；重新生成同视口打开态截图后未发现剩余 P0、P1 或 P2 问题。

### 最终检查

- 字体与排版：两项使用现有桌面菜单 `14px` 左右的响应式字号、`w500` 层级和单行文本；中文文案完整显示。
- 间距与布局：刷新与加号均为 `32dp` 控件，间距 `4dp`；菜单与加号保持 `6dp` 间距，宽度 `240dp`，两行各 `36dp`，右对齐且不遮挡标题。
- 颜色与视觉 token：按钮和菜单复用 `surface / border / secondaryText`、现有 hover/pressed 色和 overlay 阴影；没有新增独立色板。
- 图像与图标：本轮没有位图或品牌图片；刷新、加号、Command 和桌面设备均使用项目已有 Cupertino 图标库，没有手绘 SVG、字符图标或占位资产。
- 文案：加号菜单按顺序完整展示“生成 Skill Agent 安装指令”“到宿主机安装代理”；“刷新智能体列表”只作为独立刷新按钮的本地化 tooltip 与语义标签。
- 交互与可访问性：刷新按钮可直接触发列表同步；加号使用“更多操作”语义，菜单与菜单项保留 menu/menuItem 角色。Widget 测试验证独立刷新调用及两项菜单，E2E smoke 验证菜单到 Skill 指令弹窗，既有 Widget 测试验证菜单到宿主机安装弹窗。
- 边界：Flutter screenshot smoke 和自动化交互均通过；尚未在真实签名 macOS `.app` 中进行鼠标与 VoiceOver 人工验收，保持 **UNVERIFIED**。

final result: passed

---

## 我页资料卡点击入口与关系统计重构复验（2026-08-04）

> 选用 ImageGen 的蓝色细描边标签方案与轻量横排关系统计方案，并同步到 Ardot「AWiki Me · 核心UI · 重构v1」。视觉对照图和 P0110 截图只作为本地验收证据，未提交到仓库。未使用 Superpowers 技能。

### 设计与实现

- 移除我页蓝色「编辑个人资料」按钮，改为整个上层资料信息区域可点击；资料卡最右侧使用标准 chevron 图标提示可进入编辑页，点击区域与关注、粉丝入口互不嵌套。
- 标签改为透明背景、低透明度 AWiki 蓝细描边和次要文字色，不再使用实心胶囊。
- 关注和粉丝改为等宽横排入口，数字与名称保持同一基线，移除中间竖线和独立实心卡片，仅保留轻分隔线。
- 复核发现首版实现误用了内缩圆角卡片；现已按 ImageGen 参考改为全宽平铺资料区，移除外层圆角、左右外边距以及资料区与下方列表之间的卡片间隙。
- 真机继续复核发现顶部栏与资料区之间残留 40dp 页面底色，形成暖白与浅灰的横向色带；现已移除该空白，使资料区从顶部栏下沿连续开始。
- 按最终反馈将资料区表面改为与 DID / 主页 / 设置列表一致的白色，并在两组白色内容之间加入 8dp 暖灰分组间距。
- Ardot 画面 `26 我` 已在真机对齐后标记为「当前 APP」。

### 自动化、视觉与 P0110

- `flutter analyze --no-pub`、个人资料定向测试 `7/7`、`git diff --check` 和 Android arm64 Debug 构建通过。
- ImageGen 参考与 P0110 实现以同一画布并排检查；资料卡信息层级、标签轮廓、右侧箭头、横排统计和底部导航一致，未发现 P0、P1 或 P2 视觉差异。
- P0110 使用保留数据覆盖安装；点击资料卡进入「编辑个人资料」并可返回我页，点击关注进入「我关注的」，点击粉丝进入「关注我的」，两条列表返回路径均恢复我页且无白屏。
- 全宽平铺修正再次覆盖安装并与同一 ImageGen 参考并排复核，顶部资料区不再显示圆角包裹，资料、统计与下方列表的水平边界已对齐。
- 颜色连续性修正再次覆盖安装并完成同画布对照；顶部栏下方不再出现独立色带，未发现新的 P0、P1 或 P2 视觉差异。
- 白色表面统一方案经过 ImageGen 参考、Ardot 和 P0110 同画布复核；8dp 间距清晰但不过度，资料卡点击仍可进入编辑页。
- 未修改个人资料、关注关系或 APP 数据。其他 Android 尺寸、动态字体、深色模式、TalkBack、iOS 真机与正式签名发布保持 **UNVERIFIED**。
